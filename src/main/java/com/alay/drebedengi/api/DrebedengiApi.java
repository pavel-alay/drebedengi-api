...
public class DrebedengiApi {
    ...
    public List<GenericRecord> fetchDebits(LocalDate from) throws IOException {
        GetRecordListReturn response = request(new GetRecordList()
                        ...
                        .withParam("r_period", 0)// Show last 20 record (for each operation type, if not one, see 'r_what')
                        .withParam("period_from", DateTimeUtils.dateToString(from))// 'period_from' [YYYY-MM-DD] - custom period, if 'r_period' = 0;
                        .withParam("period_to", DateTimeUtils.dateToString(LocalDate.now()))// 'period_to' [YYYY-MM-DD] - custom period, if 'r_period' = 0;
                        .withParam("r_what", OperationType.DEBIT.getValue())
                        .withParam("r_how", 1)// Show in original currency
        );
